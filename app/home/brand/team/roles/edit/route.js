...
const TeamRoleEditRoute = Ember.Route.extend(AuthenticatedRouteMixin, {
  ...
  afterModel(model) {
    ...
    //Check if authorized
    let roleName = model.role.get('name').toLowerCase();
    if (roleName === 'owner' || roleName === 'guest') {
      this.replaceWith('brand.edit.team.roles.401');
    }
  },
  //actions
  actions: {
    ...
    editRole() {
      if (this.validate()) {
        ...
        roleObj.save().then(() => {
          ...
          this.transitionTo('brand.edit.team.roles', brand.get('id')); //Redirect to roles list upon adding
        }, () => {
          this.currentModel.data.set('isSubmitted', false);
        });
      }
