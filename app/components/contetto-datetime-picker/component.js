...
export default Ember.Component.extend({
    ...
    myTimeZone: 0,
    myMer: "pm",
    myDateTime: Ember.computed('myMer', 'myDate', 'myHour', 'myMinute', 'myTimeZone', {
        get() {
            var t = this.buildTime();
            var m = t.getMinutes();
            if (m < 10) {
                m = '0' + m;
            }
            ...
            return (t.getMonth() + 1) + "/" + t.getDate() + "/" + t.getFullYear() + " " + h + ":" + m + " " + a;
        }
    }),
    rfc3339: Ember.computed('myMer', 'myDate', 'myHour', 'myMinute', 'myTimeZone', {
        get() {
            var t = this.buildTime();
            return moment(t).utc().format();
        }
    }),
    myUTCDateTime: Ember.computed('myMer', 'myDate', 'myHour', 'myMinute', 'myTimeZone', {
        get() {
            var t = this.buildTime();
            ...
            return (t.getUTCMonth() + 1) + "/" + t.getUTCDate() + "/" + t.getUTCFullYear() + " " + h + ":" + m + " " + a;
        }
    }),
    myTZTime: Ember.computed('myMer', 'myDate', 'myHour', 'myMinute', 'myTimeZone', {
        get() {
            var t = this.buildTime();
            /* global moment */
            return moment(t).utcOffset(this.myTimeZone).format('YYYY-MM-DD HH:mm');
        }
    }),
    fromNow: Ember.computed('myMer', 'myDate', 'myHour', 'myMinute', 'myTimeZone', {
        get() {
            var t = this.buildTime();
    ...
    buildTime: function() {
        ...
        var y = this.myDate.getFullYear();
        var m = this.myDate.getMonth() + 1;
        if (m < 10) {
            m = '0' + m;
        }
        var d = (this.myDate.getDate());
        if (d < 10) {
            d = '0' + d;
        }
        var a = this.myMer;
        ...
        if (h >= 24) {
            h = h - 24;
            var newDate = this.addDays(this.myDate, 1);
        ...
        var S = y + "-" + m + "-" + d + " " + h + ":" + i;
        // console.log('To moment: ' + S + '/ ' + z);
        var theTime = new Date(moment.tz(S, z));
        this.get("action")(moment(theTime).utc().format());
        return theTime;
    },
    init: function() {
        var newDate = false;
        // console.log("DDT: " + this.get("defaultDT"));

        if (this.get("defaultDT") == null) {
            //   console.log("Newdate");
            newDate = true;
        }
        if (newDate) {
            ...
            var now = new Date(this.get('defaultDT'));
        }
        var d = now.getDate();
        ...
        this.set('hourOptions', []);
        this.set('minuteOptions', []);
        ...
        var noDTZ = true;
        if (this.get("defaultTimeZone") != "" && this.get("defaultTimeZone") != null) {
            this.set("myTimeZone", this.get("defaultTimeZone"));
            noDTZ = false;
        }
        if (noDTZ) {
            var mytz = moment.tz.guess();
            this.set("myTimeZone", mytz);
        }
    ...
    actions: {
        tzChanged: function(tz) {
            this.set("myTimeZone", tz);
        },
        merChanged: function(mer) {
            this.set("myMer", mer);
        },
        hourChanged: function(hour) {
            this.set("myHour", hour);
        },
        minuteChanged: function(minute) {
            this.set("myMinute", minute);
        },

        changeDateAction: function(date) {
