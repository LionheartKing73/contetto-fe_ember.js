...
export default Ember.Component.extend({
  ...
  boxOpenChats: Ember.computed('microchat.data.openChats.length', 'chatListObs', function() {
    if (this.get("microchat.data.openChats.length") > 0) {
      return this.get("microchat.data.openChats").slice(0, 2);
    }
    return [];
  }),
  listOpenChats: Ember.computed('microchat.data.openChats.length', 'chatListObs', function() {
    if (this.get("microchat.data.openChats.length") > 2) {
      return this.get("microchat.data.openChats").slice(2, 100);
    }
    return [];
  }),
  ocIDs: [],
  ...
  actions: {
    setRoom: function(room) {
      ...
      if (this.get("activeRoom.id")) {
        this.doCloseMC(this.get("activeRoom.id"));
      }
      this.set("activeRoom", room);
      this.doMicroChat(room.get('id'));
