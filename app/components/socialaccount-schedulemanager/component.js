...
function filterByNetwork(value) {
  return Ember.computed('times.@each.networkType', {
    get() {
      const collection = Ember.get(this, 'times');
      return collection.filterBy('networkType', value);
    }
  });
}
...
export default Ember.Component.extend({
  ...
  init: function() {
    this.times = [];
    let component = this;
    let nid = this.get("networkType.id");
    let account = this.get('account');
    account.get('accountSchedules').then((sched) => {
      sched.forEach(function(item) {
        if (item.get('networkType.id') == nid) {
          component.get('times').pushObject(item);
        }
      });
    });
    var zones = [];
  ...
  createTime: function(day, networkType, hour, minute) {
    if (!hour) {
      hour = 12;
    }
    if (!minute) {
    ...
    let record = this.get('store').createRecord("accountSchedule", {
      'socialAccount': this.get('account'),
      'hour': hour,
      'minute': minute,
      'dayOfWeek': day,
      'timezone': this.get('myTimezone'),
      'networkType': networkType
    });
    this.get('times').pushObject(record);
    this.get('account.accountSchedules').pushObject(record);
  ...
  clearType: function() {
    let nid = this.get('networkType.id');
    let component = this;
    var items = this.get('times'),
      list = items.toArray();
    let accountSchedule = this.get('account.accountSchedules');
  ...
  createDefaults: function() {
    let component = this;
    let nt = this.get('networkType');
    let ltName = this.get("networkType.name").toLowerCase();
    this.get('ajax').request("https://gke.contetto.io/social-accounts/v1/recommendedNetworkSchedule?network=" + this.get("account.platform")).then(function(response) {
      var defaultTimes = response[ltName];
      for (var i = 0; i < defaultTimes.length; i++) {
        var thisTime = defaultTimes[i];
        component.createTime(thisTime.dayOfWeek, nt, thisTime.hour, thisTime.minute);
      }
    });
  },
  deleteItem: function(time) {
    this.get('times').removeObject(time);
    time.deleteRecord();
    this.get('deletes').pushObject(time);
    this.get('account.accountSchedules').removeObject(time);
  },
  resetAll: function() {
    this.clearType();
    this.createDefaults();
  ...
  saveTimes: function() {
    ...
    Ember.RSVP.all(this.get('times').map(function(item) {
      return item.save();
    })).then(function() {
      Ember.RSVP.all(self.get('deletes').map(function(item) {
        return item.save();
      })).then(function() {
        self.account.save().then(() => {
          self.get('toast').success('Schedule saved.');
  ...
  actions: {
    setTZ: function(tz) {
      ...
      this.get("times").forEach((time) => {
        time.set("timezone", tz);
        //        console.log("Set tz: " + tz);
      });
    },
    deleteAll: function() {
      this.clearType();
    ...
    resetAll: function() {
      this.resetAll(this.get("networkType"));
    },
    deleteTime: function(time) {
      this.deleteItem(time);
    },
    addTime: function(day, networkTypeId) {
      this.createTime(day, networkTypeId);
    },
